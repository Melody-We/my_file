...
def Evaluate(player, chessboard):
    ...
    score = 0
    if player == 'O':
        score_dict = {1: 10, 2: 50, 3: 1000, 30: -1000}  # 圈玩家局面得分
    else:
        score_dict = {10: 10, 20: 50, 30: 1000, 3: -1000}  # X玩家局面得分
    ...
    for pos in range(len(chessboard)):
        ...
        if 0 not in next_chessboard:  # 棋盘已满
            ...
            if Evaluate(player, next_chessboard) >= 1000:  # 已达到胜利条件
                ...
                children_scores = MinMaxGameTree(player, next_chessboard, 'min',
                                                 (max_depth - 1 if max_depth != -1 else -1))  # 未达到胜利条件，向前推演
            ...
            if Evaluate(player, next_chessboard) < 0:  # min方
                ...
                children_scores = MinMaxGameTree(player, next_chessboard, 'max',
                                                 (max_depth - 1 if max_depth != -1 else -1))  # 未达到胜利条件，向前推演
...
def AlphaBetaGameTree(player, chessboard, strategy='max', max_depth=4, alpha=-9999, beta=9999):

    # MinMax博弈树推演，带AlphaBeta剪枝，裁剪不必要的推理分支。根据chessboard为player计算各着点的终局得分(达到最大搜索深度或某一方胜利)，返回[(着点,得分)]列表
    # 由于井字棋推演比较简单，可以穷尽各种情况，max_depth==-1不限制搜索深度
    scores = []      #某个某位置及得分，元组的形式存储
    for pos in range(len(chessboard)):  # 求所有子局面终局得分
        ...
        if strategy == 'max':  # max方
            if 0 not in next_chessboard or max_depth == 0 or s >= 1000:  # 棋盘已满、达到最大搜索深度，或已胜利
                ...
                children_scores = AlphaBetaGameTree(player, next_chessboard, 'min',
                                                    (max_depth - 1 if max_depth != -1 else -1), alpha,
                                                    beta)  # 未达到胜利条件，向前推演
            ...
            if 0 not in next_chessboard or max_depth == 0 or s < 0:  # min方
                ...
                children_scores = AlphaBetaGameTree(player, next_chessboard, 'max',
                                                    (max_depth - 1 if max_depth != -1 else -1), alpha,
                                                    beta)  # 未达到胜利条件，向前推演
                if children_scores:  # 该分支未被裁剪
                    children_scores.sort(key=lambda x: x[1], reverse=True)  # 选出子局面极大方所有得分中最大的一个，作为当前着点最终得分
...
def main():
    ...
    first_side = int(input('Please choose the first side: 1.Player   2.Computer: '))  # 选择先手方，1玩家，2计算机
    tag=input('Please choose ○ or X: ')
    while (first_side not in (1, 2)):
        print('You should input 1 or 2 to choose the first side.')
        first_side = int(input('Please choose the first side: 1.Player   2.Computer: '))
    if first_side == 2:  # 计算机先手，先下一子
        ...
        print('thinking...')
        time.sleep(4)
        candidates = AlphaBetaGameTree(computer, chessboard, 'max')  # 候选着点及得分列表(走各个位置的得分情况）
    ...
    while (not GameOver(chessboard)):  # 游戏主循环
        ...
        candidates = AlphaBetaGameTree(computer, chessboard, 'max')  # 候选着点及得分列表
        candidates.sort(key=lambda x: x[1], reverse=True)  # 按终局得分排序
        computer_next = candidates[0][0]
        chessboard[computer_next] = 1 if computer == 'O' else 10
        print('thinking...')
